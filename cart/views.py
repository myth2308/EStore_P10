...
def cart_detail(request):
    ...
    if request.POST.get('btnCoupon'):
        ...
        if coupon_code == 'PY10':
            ...
            messages.error(request, "Ma khong hop le")
...
def checkout(request):
    ...
    if request.POST.get('btnOrder'):
        ...
        vnp.requestData['vnp_OrderInfo'] = 'Thanh toan don hang' + str(order.id) + 'vao luc' + datetime.now().strftime('%d-%m-%Y %H:%M:%S')
        vnp.requestData['vnp_OrderType'] = 'Thanh toan hoa don'
    ...
    if inputData:
        ...
        if vnp.validate_response(settings.VNPAY_HASH_SECRET_KEY):
            if vnp_ResponseCode == '00':
                ...
                return render(request, 'cart/result.html',{
                    'result': 'Thanh toan khong thanh cong',
                    'amount': amount,
                    'order_desc': order_desc,
                    'CardType': vnp_CardType,
                    'BankCode': vnp_BankCode,
